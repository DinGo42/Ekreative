...
export const generatePageIndexes = (
  ...
  const countToLeftEnd = clamp(
    currentPage - sidewaysCount >= 0
      ? currentPage - sidewaysCount
      : sidewaysCount - (currentPage - sidewaysCount),
    0,
    sidewaysCount
  );
  ...
  const countToLeftEndAdjusted =
    countToRightEnd < sidewaysCount
      ? clamp(
          minimalLength - 1 - lastItemsCount,
          0,
          minimalLength - lastItemsCount - countToRightEnd
        )
      : countToLeftEnd;

  Array.from({ length: countToLeftEndAdjusted + countToRightEnd + 1 }).forEach(
    (_, index) => {
      const adjustedCurrentPage =
        currentPage > totalPages - lastItemsCount
          ? totalPages - lastItemsCount
          : currentPage;
      result.push(adjustedCurrentPage - countToLeftEndAdjusted + index);
    }
  );

  Array.from({ length: lastItemsCount }).forEach((_, index) =>
